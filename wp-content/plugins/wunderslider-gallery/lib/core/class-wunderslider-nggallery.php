	...
	public static function plugins_loaded() {
		if ( class_exists( 'nggdb' ) ) {
			...
			if ( self::handle_shortcodes() ) {
				if ( method_exists( 'C_NextGen_Shortcode_Manager', 'remove' ) ) {
					C_NextGen_Shortcode_Manager::remove( 'nggallery' );
					C_NextGen_Shortcode_Manager::add( 'nggallery', array( __CLASS__ , 'nggallery' ) );
				} else {
	...
	public static function nggallery( $attr, $content = null ) {
		...
		if ( $handle && $use_ws ) {
			...
			if ( function_exists( 'nggShowGallery' ) ) {
				ob_start();
				$ngg = nggShowGallery( $id, $template, $images );
			...
			} else if ( method_exists( 'M_NextGen_Basic_Gallery', 'render' ) ) {
				$g = new M_NextGen_Basic_Gallery();
				return $g->render( $attr );
			} else {
				return __( 'ERROR : Could not render gallery due to missing functions.', WSG_PLUGIN_DOMAIN );
			}
		}
	}
}
