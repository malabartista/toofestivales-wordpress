	...
	public static function post_gallery( $empty = '', $attr = array() ) {
		...
		extract(
			shortcode_atts(
				array(
					...
					'include'    => '',
					'exclude'    => '',
					'linkurls'   => false // if this should be enabled, it requires useFlick="false" clickable="true" to be set explicitly
				),
				$attr
			)
		);
		$linkurls = ( $linkurls === true ) || ( $linkurls == 'true' );
		...
		foreach( $attachments as $id => $attachment ) {
			if ( wp_attachment_is_image( $id ) ) {
				...
				$xml .= sprintf( '<image url="%s" title="%s" description="%s" %s />', esc_attr( $url ), esc_attr( $title ), esc_attr( $description ), $linkurls ? ' linkUrl="' . esc_url( $url ) . '" ' : '' );
			}
		}
