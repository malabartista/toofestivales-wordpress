...
<?php
	...
	if (defined( 'WS_PLUGIN_DIR' ) ) {
		if ( is_dir( WS_PLUGIN_DIR . '/css' ) ) {
			if ( $entries = scandir( WS_PLUGIN_DIR . '/css' ) ) {
				$entries = array_diff( $entries, array( '.', '..' ) );
				foreach( $entries as $entry ) {
					if ( is_dir( WS_PLUGIN_DIR . '/css/' . $entry ) ) {
						$entry = wp_strip_all_tags( trim ( $entry ) );
						if ( $skin == $entry ) {
							$selected = ' selected="selected" ';
						} else {
							$selected = '';
						}
						if ( !key_exists( $entry, WSG_Utility::$skins ) ) {
							printf( '<option %s value="%s">%s</option>', $selected, esc_attr( $entry ), $entry );
						}
					} 
				}
			}
