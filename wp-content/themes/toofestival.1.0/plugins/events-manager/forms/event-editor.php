...
<form enctype='multipart/form-data' id="event-form" method="post" action="<?php echo esc_url(add_query_arg(array('success'=>null))); ?>">
	<div class="wrap">
		...
		<?php if(get_option('dbem_events_anonymous_submissions') && !is_user_logged_in()): ?>
			<h3 class="event-form-submitter"><?php esc_html_e( 'Your Details', 'events-manager'); ?></h3><hr>
		...
		<?php endif; ?>
		<h3 class="event-form-name"><?php esc_html_e( 'Event Name', 'events-manager'); ?></h3><hr>
		<div class="inside event-form-name">
			<input type="text" name="event_name" id="event-name" value="<?php echo esc_attr($EM_Event->event_name,ENT_QUOTES); ?>" placeholder="<?php esc_html_e('The event name. Example: Birthday party', 'events-manager'); ?>"/><?php echo $required; ?>
			<br />
			<?php em_locate_template('forms/event/group.php',true); ?>
		</div>
					
		<h3 class="event-form-when"><?php esc_html_e( 'When', 'events-manager'); ?></h3><hr>
		...
		<h3 class="event-form-where"><?php esc_html_e( 'Where', 'events-manager'); ?></h3><hr>
	...
	<p class="submit">
	    ...
	    <input type='submit' class='btn btn-primary btn-lg' value='<?php echo esc_attr(sprintf( __('Submit %s','events-manager'), __('Event','events-manager') )); ?>' />
	    <?php else: ?>
	    <input type='submit' class='btn btn-primary btn-lg' value='<?php echo esc_attr(sprintf( __('Update %s','events-manager'), __('Event','events-manager') )); ?>' />
	    <?php endif; ?>
	</p>
